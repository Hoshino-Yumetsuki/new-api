...
func assertResponsesSSETerminal(t *testing.T, body []byte, terminal string) {
	t.Helper()
	var expected map[string]any
	require.NoError(t, common.UnmarshalJsonStr(terminal, &expected))
	if response, ok := expected["response"].(map[string]any); ok {
		response["model"] = "ws-billing"
	}
	terminals := 0
	for line := range strings.SplitSeq(string(body), "\n") {
		...
		if event["type"] == expected["type"] {
			terminals++
			assert.Equal(t, expected, event, "SSE preserves the terminal payload while restoring the client model")
		}
	}
	assert.Equal(t, 1, terminals, "the terminal event must be delivered exactly once")
}
...
func TestResponsesWebSocketDialsNativeResponsesChannelTypes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assertResponsesSSETerminal(t, body, terminal)
			select {
			case <-fixture.httpDone:
			case <-time.After(3 * time.Second):
