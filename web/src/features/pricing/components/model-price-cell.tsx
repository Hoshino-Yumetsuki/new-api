...
export function ModelPriceCell(props: {
  ...
  if (dynamic) {
    ...
    let primaryEntries = dynamic.primaryEntries
    if (!dynamic.isTaskUsage && !dynamic.isTimePricing) {
      primaryEntries = [
        ...dynamic.marketplaceEntries.filter(
          (entry) => entry.key === 'p' || entry.key === 'c'
        ),
        ...dynamic.primaryEntries.filter(
          (entry) => entry.unit === 'request' || entry.unit === 'image'
        ),
      ]
    }
    const hasRequestPrice = primaryEntries.some(
      (entry) => entry.unit === 'request' || entry.unit === 'image'
    )
    metrics = primaryEntries.slice(0, hasRequestPrice ? 3 : 2).map((entry) => {
