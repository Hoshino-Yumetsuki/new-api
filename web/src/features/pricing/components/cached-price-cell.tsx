...
export function CachedPriceCell(props: {
  ...
  if (dynamicSummary) {
    ...
    const cacheEntries = dynamicSummary.entries.filter(
      (entry) => entry.field === 'imageCachePrice'
    )
    const genericCacheEntry = dynamicSummary.isTimePricing
      ? dynamicSummary.entries.find((entry) => entry.field === 'cacheReadPrice')
      : dynamicSummary.marketplaceEntries.find((entry) => entry.key === 'cache')
    if (genericCacheEntry) cacheEntries.unshift(genericCacheEntry)
