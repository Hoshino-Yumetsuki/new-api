...
const verificationScopeLoginMigration = "auth.login.legacy_github_migration"
...
func StartLoginVerification(user *model.User, loginMethod string, migration *LegacyGitHubMigration) (*LoginChallenge, error) {
	...
	verificationScope := VerificationScopeLogin
	if migration != nil {
		verificationScope = verificationScopeLoginMigration
	}
	methods, err := securityVerificationPolicy(verificationScope, *state)
...
func RequireLoginVerification(token, method string) (*LoginVerification, error) {
	...
	if err := requireLoginVerificationMethod(state, method, payload.PendingGitHubID != ""); err != nil {
		return nil, err
	}
	return &LoginVerification{Flow: flow, State: state, payload: payload}, nil
}

func requireLoginVerificationMethod(state *model.UserVerificationState, method string, migration bool) error {
	verificationScope := VerificationScopeLogin
	if migration {
		verificationScope = verificationScopeLoginMigration
	}
